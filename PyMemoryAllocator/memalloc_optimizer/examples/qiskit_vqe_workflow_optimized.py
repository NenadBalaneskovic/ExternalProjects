...
# ============================================================
# 1. Optimized expectation value (Numba JIT)
# ============================================================
...
def run_vqe():
    ...
    for step in range(20):
        ...
        # Qiskit 1.x: assign_parameters replaces bind_parameters
        ...
        # Gradient evaluation also must use assign_parameters
        bound_grad = circuit.assign_parameters({theta: theta_val + 1e-3})
        grad_state = np.array(execute(bound_grad, backend).result().get_statevector())

        grad = (expectation_numba(grad_state, coeffs, paulis) - exp_val) / 1e-3
